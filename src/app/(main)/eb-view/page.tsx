...
import ReadingsSummaryModal from '@/components/ReadingsSummaryModal'; // This is the new component
...
// --- EBReadingCard COMPONENT (No changes needed here) ---
const EBReadingCard: FC<EBReadingCardProps> = ({ reading, nextDayMorningUnits, onUpdate, onImageZoom, onHistoryOpen }) => {
    // ... (This component remains exactly the same as in your original post)
...
export default function EBViewPage(): JSX.Element {
    const { data: session } = useSession() as { data: CustomSession | null };
    // ** MODIFIED **: Use the new combined data type for state
    ...
    // ** MODIFIED **: The data fetching function is now updated to fetch and merge both datasets.
    const fetchPageData = useCallback(async () => {
        setIsLoading(true);
        try {
            // Fetch both datasets concurrently for better performance
            const [ebRes, appointmentsRes] = await Promise.all([
                fetch('/api/eb'),
                fetch('/api/appointment/summary') // Fetch from the new API
            ]);
            ...
            if (!ebData.success) {
                throw new Error(ebData.message || 'Failed to fetch EB readings.');
            }

            // Get the appointment counts map: { 'YYYY-MM-DD': count }
            const appointmentCounts = appointmentsData.success ? appointmentsData.counts : {};

            // Merge the appointment counts into the EB readings
            const combinedData: IEBReadingWithAppointments[] = (ebData.readings as IEBReading[]).map(reading => {
                // Format the date to 'YYYY-MM-DD' to match the key from our new API
                const dateKey = new Date(reading.date).toISOString().split('T')[0];
                return {
                    ...reading,
                    appointmentCount: appointmentCounts[dateKey] || 0 // Assign count, or 0 if none
                };
            });
    ...
    useEffect(() => {
        if(canViewCalculateEB) {
            fetchPageData(); // Call the updated fetch function
        } else if (session) {
            setIsLoading(false);
        }
    ...
    const handleUnitUpdate = async (id: string, morningUnits: number | undefined) => {
      try {
        ...
        await fetchPageData(); // Refetch all data after update
      } catch (error) {
    ...
    const handleSetGlobalCost = async (newCost: number) => {
        ...
        try {
            const updatePromises = readings.map(reading => fetch('/api/eb', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ readingId: reading._id, morningUnits: reading.morningUnits, costPerUnit: newCost })
            }));
    ...
    return (
        <>
            ...
            {/* ** MODIFIED **: The modal now receives the enriched `readings` data automatically */}
            ...
            <main className=" bg-slate-100 min-h-screen">
                <div className="max-w-7xl mx-auto space-y-8 p-4 sm:p-6 lg:p-8">
                    {/* ... Header and buttons section remains the same ... */}
                    <div className="flex flex-wrap justify-between items-center gap-4">
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            <button onClick={() => setIsReportModalOpen(true)} className="text-sm bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 font-medium flex items-center px-4 py-2 rounded-lg shadow-sm transition-colors">
                    ...
                    {/* ... The rest of the JSX rendering the cards remains the same ... */}
