...
//================================================================================
//  Reusable Presentational Components (Minor padding adjustments for mobile)
//================================================================================
...
const StatCard: FC<any> = ({ title, value, icon: Icon, color = 'blue', onClick }) => {
  ...
  // RESPONSIVE: Use smaller padding on mobile (p-4) and larger on bigger screens (sm:p-6)
...
const QuickActionCard: FC<any> = ({ title, description, icon: Icon, onClick }) => (
  // RESPONSIVE: Use smaller padding on mobile (p-4) and larger on bigger screens (sm:p-6)
  <div onClick={onClick} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 hover:shadow-md transition-all cursor-pointer group hover:border-gray-300">
    <div className="flex items-start space-x-4">
      <div className="p-2 bg-gray-50 rounded-lg group-hover:bg-gray-100 transition-colors"><Icon className="h-6 w-6 text-gray-600" /></div>
...
const SalesStatCard: FC<{ title: string; value: number; icon: React.ElementType; color?: string; isCurrency?: boolean; }> = ({ title, value, icon: Icon, color = 'bg-gray-100 text-gray-600', isCurrency = false }) => (
  // RESPONSIVE: Use smaller padding on mobile (p-4)
  <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex items-center space-x-4">
    ...
    <div>
      ...
      <div className="text-2xl font-bold text-gray-900"><AnimatedNumber value={value} prefix={isCurrency ? '₹' : ''} decimals={isCurrency ? 2 : 0} /></div>
    </div>
  </div>
);
...
export default function DashboardPage() {
  ...
  if (session && !isOverviewLoading && !canReadDashboard) {
    // ... (This access denied block is unchanged and already responsive)
    return (
      <div className="flex items-center justify-center h-screen bg-gray-50 p-4">
        <div className="text-center bg-white p-10 rounded-xl shadow-md border border-red-200">
          ...
          <p className="mt-2 text-gray-600">You do not have the required permissions to view the dashboard.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    // RESPONSIVE: The root container padding is already responsive. `space-y-8` provides good vertical rhythm.
    <div className="space-y-8 min-h-screen">
      {/* SECTION 1: OVERVIEW */}
      <div>
        {/* RESPONSIVE: `flex-col sm:flex-row` stacks the title and actions on mobile and puts them side-by-side on larger screens. Perfect. */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
          <div>
            {/* RESPONSIVE: Font size shrinks on mobile (`text-2xl`) and grows on larger screens (`md:text-3xl`). Excellent pattern. */}
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Welcome back, {session?.user.name}! 👋</h1>
            <p className="text-gray-600 mt-1">Here's what's happening at your salon today.</p>
          </div>
        </div>
        
        {isOverviewLoading ? (
          <div className="animate-pulse mt-6"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6"><div className="h-32 bg-gray-200 rounded-xl"></div><div className="h-32 bg-gray-200 rounded-xl"></div><div className="h-32 bg-gray-200 rounded-xl"></div><div className="h-32 bg-gray-200 rounded-xl"></div></div></div>
        ) : (
          <div className="mt-6 space-y-6">
            {/* RESPONSIVE: This grid is the core of responsiveness. 1 col on mobile, 2 on tablet, 4 on desktop. Perfect. */}
            ...
            {/* RESPONSIVE: Smaller padding for the card on mobile */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Quick Actions</h2>
              {/* RESPONSIVE: 1 col on mobile, 2 on tablet/desktop. Perfect. */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <QuickActionCard title="Book Appointment" description="Schedule a new appointment for a customer" icon={CalendarDaysIcon} onClick={() => window.location.href = '/appointment'} />
                <QuickActionCard title="Add Customer" description="Register a new customer in the system" icon={UserGroupIcon} onClick={() => window.location.href = '/crm'} />
              </div>
            </div>
          </div>
        )}
      ...
      <div className="p-4 sm:p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        <div className="bg-gray-50 p-4 rounded-xl border mb-6">
          {/* RESPONSIVE: `flex-col sm:flex-row` ensures date pickers stack vertically on mobile. `items-stretch sm:items-center` makes them full-width and aligned on mobile. */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 flex-1">
              <label htmlFor="startDate" className="text-sm font-medium sm:mr-2 shrink-0">From:</label>
              <input type="date" name="startDate" value={dateRange.startDate} onChange={handleDateChange} className="p-2 border rounded-md text-sm w-full" />
            </div>
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 flex-1">
              <label htmlFor="endDate" className="text-sm font-medium sm:mr-2 shrink-0">To:</label>
              <input type="date" name="endDate" value={dateRange.endDate} onChange={handleDateChange} className="p-2 border rounded-md text-sm w-full" />
            </div>
          </div>
        </div>
        ...
        {isReportLoading ? <div className="text-center p-10 font-semibold text-gray-600">Loading Report...</div> : !salesReportData ? <div className="text-center p-10 text-red-500 bg-red-50 rounded-lg">Could not load sales report. Please try a different date range.</div> : (
          <div className="space-y-6">
            {/* RESPONSIVE: This grid is already perfectly responsive. */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
