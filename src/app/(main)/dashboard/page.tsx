...
import AnimatedNumber from '@/components/AnimatedNumber'; // Assuming you have this component
...
import {
  ...
  ScaleIcon,
  CurrencyRupeeIcon,
  ShieldExclamationIcon, // Icon for the access denied message
...
//================================================================================
//  Reusable Presentational Components (These are self-contained and need no changes)
//================================================================================
...
export default function DashboardPage() {
  ...
  // 2. DERIVE PERMISSIONS AT THE TOP OF THE COMPONENT
  ...
  // --- Tenant-Aware Fetch Helper (This is correct and robust) ---
  ...
  // --- Data Fetching for Overview Section ---
  useEffect(() => {
    ...
    // Only fetch data if the user has a session AND has permission.
    if (session && canReadDashboard) {
        fetchDashboardData();
    } else if (session) {
        // If logged in but no permission, stop the loading spinner.
        setIsOverviewLoading(false);
    }
  }, [session, canReadDashboard, tenantFetch]);

  // --- Data Fetching for Sales Report Section ---
  const fetchSalesReportData = useCallback(async () => {
    if (!session || !canReadDashboard) return; // Guard clause
  ...
  // 3. IMPLEMENT THE PERMISSION GATE
  // This renders after initial session check but before showing the dashboard.
  ...
  // This is the main return block. It will only render if the permission check passes.
  return (
    <div className="space-y-8 p-4 md:p-6 bg-gray-50 min-h-screen">
      {/* SECTION 1: OVERVIEW */}
      <div>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
          <div><h1 className="text-2xl md:text-3xl font-bold text-gray-900">Welcome back, {session?.user.name}! 👋</h1><p className="text-gray-600 mt-1">Here's what's happening at your salon today.</p></div>
        </div>
