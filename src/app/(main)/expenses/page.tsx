...
function ExpenseDetailsModal({ isOpen, onClose, title, historyData, onEdit, onDelete }: ExpenseDetailsModalProps) {
  ...
  const handleViewBill = (billUrl: string) => {
    ...
    else if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg'].includes(extension)) fileType = `image/${extension}`;
  ...
  const handlePdfDownload = () => {
    ...
    const tableColumn = ["Date", "Category", "Sub-Category", "Description", "Payment", "Amount", "Bill"];
    const tableRows: (string | number)[][] = historyData.map(expense => [
  ...
  const handleCsvDownload = () => {
    ...
    const tableColumn = ["Date", "Category", "Sub-Category", "Description", "Frequency", "Payment Method", "Amount", "Bill URL"];
    ...
    historyData.forEach(expense => {
      const row = [
        ...
        escapeCsvCell(expense.subCategory),
                    ...
                    <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                        <Dialog.Panel className="w-full max-w-7xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all border-t-4 border-gray-800">
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200">
                                        <thead className="bg-gray-100 sticky top-0 z-10">
                                            <tr>
                                                ...
                                                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sub-Category / Item</th>
                                            ...
                                            {historyData.map(expense => (
                                                <tr key={expense._id} className="hover:bg-gray-100">
                                                    <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-600">
                                                        {new Date(expense.date).toLocaleDateString('en-GB')}
                                                        <span className="block text-xs text-gray-400">{new Date(expense.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true })}</span>
                                                    </td>
                                                    <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-800">{expense.category}</td>
                                                    <td className="px-4 py-4 text-sm text-gray-600 max-w-xs truncate" title={expense.subCategory}>{expense.subCategory}</td>
                                                    ...
                                                    <td className="px-4 py-4 whitespace-nowrap text-center text-sm">
                                                        {expense.billUrl ? (
                                                            <button onClick={() => handleViewBill(expense.billUrl!)} className="inline-flex items-center gap-1.5 text-indigo-600 hover:text-indigo-800 font-medium transition-colors"><EyeIcon className="h-4 w-4"/> View</button>
                                                        ) : ( <span className="text-gray-400">N/A</span> )}
                                                    </td>
                                                    <td className="px-4 py-4 whitespace-nowrap text-center text-sm font-medium">
                                                        <div className="flex justify-center items-center gap-4">
                                                            <button onClick={() => onEdit(expense)} className="text-blue-600 hover:text-blue-800 transition-colors flex items-center gap-1"><PencilIcon className="h-4 w-4" /> Edit</button>
                                                            <button onClick={() => onDelete(expense._id)} className="text-red-600 hover:text-red-800 transition-colors flex items-center gap-1"><TrashIcon className="h-4 w-4" /> Delete</button>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))}
                            ...
                            <div className="mt-6 flex justify-end items-center gap-3">
                                <button onClick={handleCsvDownload} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-md hover:bg-gray-200 transition-colors"><ArrowDownTrayIcon className="h-4 w-4" /> Export to Excel (CSV)</button>
                                <button onClick={handlePdfDownload} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-gray-800 rounded-md hover:bg-gray-900 transition-colors"><ArrowDownTrayIcon className="h-4 w-4" /> Download PDF</button>
                            </div>
                        </Dialog.Panel>
                    </Transition.Child>
                </div>
                ...
                <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                  <Dialog.Panel className="w-full max-w-4xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                    <Dialog.Title as="h3" className="text-lg font-medium leading-6 text-gray-900 flex justify-between items-center">
                      ...
                      <button onClick={onClose} className="p-1 rounded-full hover:bg-gray-200 transition-colors"><XMarkIcon className="h-6 w-6 text-gray-600" /></button>
                    </Dialog.Title>
                    <div className="mt-4">
...
export default function ExpensesPage() {
      ...
      const fetchAllData = async () => {
        ...
        try {
          const [expensesResponse, budgetCategoriesResponse] = await Promise.all([
            ...
            api.get('/api/budget-categories', session)
          ]);
          ...
          if (expensesResponse.success) {
            ...
            setPaymentMethods(Array.from(new Set(['Cash', 'UPI', 'Card', ...fetchedMethods])));
          } else {
            throw new Error(expensesResponse.error || "Failed to fetch expenses.");
          }

          if (budgetCategoriesResponse.success) {
            setBudgetCategoryList(budgetCategoriesResponse.data);
          } else {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    else if (existingBillUrl) formData.append('billUrl', existingBillUrl);
    ...
    try {
      isEditing
        ? await api.put(`/api/expenses/${editingExpenseId}`, formData, session)
        : await api.post('/api/expenses', formData, session);
  ...
  const handleAddPaymentMethod = () => {
    ...
    if (!trimmedNewMethod) { 
        setPaymentMethodError("Method name cannot be empty."); 
        return; 
    }
    if (paymentMethods.some(pm => pm.toLowerCase() === trimmedNewMethod.toLowerCase())) { 
  ...
  const todayStats = useMemo(() => {
    ...
    if (todaysExpenses.length > 0) {
        const categoryCounts = todaysExpenses.reduce((acc, exp) => {
            acc[exp.category] = (acc[exp.category] || 0) + 1;
            return acc;
        }, {} as Record<string, number>);
        mostCommonCategory = Object.keys(categoryCounts).reduce((a, b) => categoryCounts[a] > categoryCounts[b] ? a : b, 'N/A');
    }
  ...
  const filteredExpenses = useMemo(() => {
    return allExpenses.filter(expense => {
      ...
      const typeMatch = filterType === 'all' || expense.category === filterType;
  ...
  const getHistoryItemTitle = (dateString: string, view: 'Daily' | 'Weekly' | 'Monthly') => {
            ...
            <form onSubmit={handleSubmit} className="space-y-4 flex-grow flex flex-col">
              ...
              <div>
                  <div className="flex justify-between items-baseline">
                      ...
                      <button type="button" onClick={() => setShowAddPaymentMethod(!showAddPaymentMethod)} className="text-xs font-semibold text-gray-600 hover:text-gray-900">
                          {showAddPaymentMethod ? 'Cancel' : 'Add New'}
                      </button>
                  </div>
                  <select id="paymentMethod" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-gray-500 focus:border-gray-500">
              ...
              <div className="mt-auto flex items-center gap-4">
                <button type="submit" className="w-full bg-gray-800 text-white py-2.5 px-4 rounded-md hover:bg-gray-900 font-semibold transition-colors">{editingExpenseId ? 'Update Expense' : 'Submit Expense'}</button>
                {editingExpenseId && (<button type="button" onClick={resetForm} className="w-full bg-gray-200 text-gray-800 py-2.5 px-4 rounded-md hover:bg-gray-300 font-semibold transition-colors">Cancel</button>)}
              </div>
            </form>
          </div>
