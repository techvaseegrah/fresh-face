...
export default function GiftCardRedemptionReportPage() {
    ...
    return (
        <div className="p-3 sm:p-6 bg-gray-50 min-h-screen">
            <h1 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4">Gift Card Redemption Report</h1>
            ...
            <div className="bg-white rounded-lg shadow-md">
                ...
                <div className="block md:hidden">
                    {reportData.length > 0 ? (
                                ...
                                <div key={item._id} className="p-4 hover:bg-gray-50 transition-colors active:bg-gray-100">
                                    ...
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1">
                                                <h3 className="font-bold text-gray-900 text-base leading-tight">{item.giftCardName}</h3>
                                                <p className="text-sm text-gray-500 mt-1">🗓️ {formatDate(item.redemptionDate)}</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <p className="font-bold text-xl text-green-600">₹{item.amountRedeemed}</p>
                                    ...
                                    <div className="space-y-3 bg-gray-50 rounded-lg p-3">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span className="text-gray-900 font-mono text-sm bg-white px-2 py-1 rounded border">{item.giftCardNumber}</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="text-gray-600 font-medium text-sm">👤 Guest Name</span>
                                            <span className="text-gray-900 font-semibold text-sm">{item.guestName}</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="text-gray-600 font-medium text-sm">📞 Guest Number</span>
                                            <span className="text-gray-900 font-semibold text-sm">{item.guestNumber}</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="text-gray-600 font-medium text-sm">📝 Remark</span>
                                            <span className="text-gray-500 italic text-sm">No remarks</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="text-center py-12">
                            ...
                            <h3 className="text-lg font-medium text-gray-900 mb-2">No Gift Cards Redeemed</h3>
                            <p className="text-gray-500 text-sm">No redemptions found for the selected period.</p>
                        </div>
                    )}
                </div>
                ...
                {!isLoading && reportData.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-500">No gift card redemptions found for the selected period.</p>
                    </div>
                )}
            </div>
        </div>
