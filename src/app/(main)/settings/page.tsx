...
// --- NEW --- Sub-component for Loyalty Settings Form ---
const LoyaltySettingsForm: React.FC = () => {
    const [settings, setSettings] = useState<LoyaltySettings>({ loyaltyPointPerPrice: 100, loyaltyPointsAwarded: 1 });
    const [isSaving, setIsSaving] = useState(false);
    const [isLoading, setIsLoading] = useState(true);
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            ...
            try {
                ...
                if (result.success && result.data) {
                    setSettings({
                        // Use default values if the fields don't exist in the DB yet
                        loyaltyPointPerPrice: result.data.loyaltyPointPerPrice || 100,
                        loyaltyPointsAwarded: result.data.loyaltyPointsAwarded || 1,
                    });
                } else {
                    console.error("Failed to fetch settings:", result.error);
                }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload = {
                loyaltyPointPerPrice: settings.loyaltyPointPerPrice,
                loyaltyPointsAwarded: settings.loyaltyPointsAwarded,
            };
            const response = await fetch('/api/settings', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    return (
        <div>
            <h2 className="text-xl font-semibold text-gray-900 mb-2">Loyalty Points Management</h2>
            <p className="text-sm text-gray-600 mb-8">Set how customers earn loyalty points from their purchases.</p>
            <form onSubmit={handleSubmit} className="max-w-md">
                <div className="space-y-6">
                    <div>
                        <label htmlFor="loyaltyPointPerPrice" className="block text-sm font-medium text-gray-700">Price for Points (₹)</label>
                        <input type="number" id="loyaltyPointPerPrice" name="loyaltyPointPerPrice" value={settings.loyaltyPointPerPrice} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-black focus:border-black text-gray-900" />
                        <p className="text-xs text-gray-500 mt-1">The amount a customer needs to spend to earn loyalty points.</p>
                    </div>
                    <div>
                        <label htmlFor="loyaltyPointsAwarded" className="block text-sm font-medium text-gray-700">Points Awarded</label>
                        <input type="number" id="loyaltyPointsAwarded" name="loyaltyPointsAwarded" value={settings.loyaltyPointsAwarded} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-black focus:border-black text-gray-900" />
                        <p className="text-xs text-gray-500 mt-1">The number of loyalty points awarded for the price specified above.</p>
                    </div>
                </div>
                <div className="mt-8 flex justify-start">
                    <Button type="submit" disabled={isSaving || isLoading} className="bg-black hover:bg-gray-800 focus:ring-gray-500">
...
const SettingsPage = () => {
    ...
    const settingsTabs = [
        ...
        { id: 'loyalty', label: 'Loyalty Points Management' }, // New Tab
    ...
    const renderContent = () => {
        ...
        switch (activeTab) {
            ...
            case 'loyalty': return <LoyaltySettingsForm />; // New Case
            default: return (
                <div>
