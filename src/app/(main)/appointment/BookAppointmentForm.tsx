...
export default function BookAppointmentForm({ isOpen, onClose, onBookAppointment }: BookAppointmentFormProps) {
  ...
  const handleUpdateAssignment = (_tempId: string, updates: Partial<ServiceAssignmentState>) => {
    setServiceAssignments(prev => prev.map(a => (a._tempId === _tempId ? { ...a, ...updates } : a)));
  };
  
  useEffect(() => {
    if (isOpen && formData.date && formData.time) {
      setIsLoadingStaff(true);
      setServiceAssignments(prev => prev.map(a => ({ ...a, isLoadingStylists: true, stylistId: '' })));
      const fetchAssignableStaff = async () => {
        try {
          ...
          setServiceAssignments(prev => prev.map(a => ({ ...a, availableStylists: staffList, isLoadingStylists: false })));
        } catch (error) {
          toast.error("Error fetching staff list.");
          setAssignableStaff([]);
          setServiceAssignments(prev => prev.map(a => ({ ...a, availableStylists: [], isLoadingStylists: false })));
        } finally {
      ...
      setAssignableStaff([]);
      setServiceAssignments(prev => prev.map(a => ({ ...a, availableStylists: [], isLoadingStylists: true, stylistId: '' })));
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
      ...
      if (serviceAssignments.length === 0) { setFormError('Please add at least one service.'); return; }
      if (serviceAssignments.some(a => !a.stylistId)) { setFormError('A staff member must be assigned to every service.'); return; }
      setIsSubmitting(true);
      try {
          const checkPayload = { serviceIds: serviceAssignments.map(a => a.serviceId), customerGender: formData.gender as 'male' | 'female' | 'other' };
