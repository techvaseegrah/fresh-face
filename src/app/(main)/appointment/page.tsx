// app/(main)/appointment/page.tsx - COMPLETE, CORRECTED, AND MORE ROBUST VERSION
...
// Interfaces for data received on this page
...
export default function AppointmentPage() {
  ...
  const handleFinalizeBill = async (appointmentId: string, finalTotal: number, billDetails: any) => {
    if (!selectedAppointmentForBilling) {
      toast.error("Critical error: No appointment selected for billing.");
      throw new Error("Critical error: No appointment selected for billing.");
    }
    ...
    const payload = {
      ...
      subtotal: billDetails.originalTotal,
      membershipDiscount: billDetails.membershipSavings,
      paymentMethod: billDetails.paymentMethod,
      notes: billDetails.notes,
      customerWasMember: billDetails.customerWasMember,
      membershipGrantedDuringBilling: billDetails.membershipGranted,
    };
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.message || 'Failed to create invoice. API returned an error.');
      }
    ...
    <div className="min-h-screen bg-gray-50/30 p-4 md:p-0">
                ...
                {allAppointments.map((appointment) => {
                  ...
                  return (
                    <tr key={appointment.id} className="bg-white border-b hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        {appointment.status === "Appointment" && appointment.appointmentTime ? (
                            <>
                              ...
                              <div className="text-xs text-gray-500">{formatTime(appointment.appointmentTime.split('T')[1].substring(0, 5))}</div>
                            </>
                        ) : (
                          <span>-</span>
                        )}
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${appointment.appointmentType === 'Online' ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800'}`}>
                      ...
                      {/**************************************************************
                       *  FIXED: Robust amount display logic.
                       *  - Differentiates between final and estimated amounts.
                       *  - Displays final amounts in green with savings.
                       *  - Displays estimated amounts in standard color.
                       *  - Shows a dash '-' for zero or null amounts.
                       **************************************************************/}
                      <td className="px-6 py-4">
                        {(() => {
                          const displayAmount = appointment.finalAmount ?? appointment.amount;
                          const isFinal = appointment.finalAmount != null;

                          if (displayAmount != null && displayAmount > 0) {
                            return (
                              <div>
                                <div className={`font-semibold ${isFinal ? 'text-green-600' : 'text-gray-900'}`}>
                                  ₹{displayAmount.toFixed(2)}
                                </div>
                                {isFinal && Number(appointment.membershipDiscount) > 0 ? (
                                  <div className="text-xs text-green-500">
                                    Saved ₹{Number(appointment.membershipDiscount).toFixed(2)}
                                  </div>
                                ) : null}
                              </div>
                            );
      ...
      {selectedAppointmentForBilling && isBillingModalOpen && (
        <BillingModal
          ...
          appointment={{
            id: selectedAppointmentForBilling._id,
            serviceIds: selectedAppointmentForBilling.serviceIds,
          }}
