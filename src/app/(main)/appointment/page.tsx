// app/appointment/page.tsx - FINAL CORRECTED VERSION
'use client';
...
import {
  ...
  ChevronRightIcon,
  PencilIcon,
  CheckCircleIcon // 1. IMPORT THE CHECK ICON
...
export default function AppointmentPage() {
  ...
  // All state and handlers are correct and remain the same
  ...
  const handleFinalizeBill = async (billDetails: any) => {
    ...
    try {
      const finalPayload = { appointmentId: selectedAppointmentForBilling._id, customerId: selectedAppointmentForBilling.customerId._id, stylistId: selectedAppointmentForBilling.stylistId._id, ...billDetails };
      const response = await fetch(`/api/billing`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(finalPayload) });
      const result = await response.json();
                ...
                {allAppointments.map((appointment) => {
                  ...
                  return (
                    <tr key={appointment.id} className="bg-white border-b hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">{appointment.finalAmount ? (<div><div className="font-semibold text-green-600">₹{appointment.finalAmount.toFixed(2)}</div>{appointment.membershipDiscount > 0 && (<div className="text-xs text-green-500">Saved ₹{appointment.membershipDiscount.toFixed(2)}</div>)}</div>) : (<span className="text-gray-400">-</span>)}</td>
                      <td className="px-6 py-4">{billingStaffName}</td>
                      <td className="px-6 py-4">{paymentSummary}</td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end space-x-2">
                          {/* 2. USE A TERNARY OPERATOR FOR CONDITIONAL RENDERING */}
