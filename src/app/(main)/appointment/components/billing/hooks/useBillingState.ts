...
export const useBillingState = ({ isOpen, appointment, customer, stylist, onFinalizeAndPay }: UseBillingStateProps) => {
  ...
  const fetchBusinessDetails = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/settings/business-details');
      const data = await res.json();
      if (data.success && data.details) { setBusinessDetails(data.details); } 
      else { setBusinessDetails({ name: 'Your Salon Name', address: '123 Beauty Street, Your City', phone: '9876543210' }); }
    } catch (err) {
      console.error('Failed to fetch business details:', err);
      setBusinessDetails({ name: 'Your Salon Name', address: '123 Beauty Street, Your City', phone: '9876543210' });
    } finally { setIsLoadingBusinessDetails(false); }
  }, []);
  ...
  const fetchInventoryImpact = useCallback(async (currentBillItems: BillLineItem[]) => {
    ...
    if (serviceItems.length === 0 || !customer._id) { setInventoryImpact(null); return; }
    setIsLoadingInventory(true);
    try {
      const serviceIds = serviceItems.map(s => s.itemId);
      const response = await tenantFetch('/api/billing/inventory-preview', { method: 'POST', body: JSON.stringify({ serviceIds, customerId: customer._id }) });
      const data = await response.json();
      if (data.success) setInventoryImpact(data.data);
    } catch (err) { console.error('Failed to fetch inventory impact:', err); } 
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      if (isCorrectionMode && appointment.invoiceId) {
        ...
        try {
          const invoiceIdToFetch = typeof appointment.invoiceId === 'object' ? (appointment.invoiceId as { _id: string })._id : appointment.invoiceId;
    ...
    initialize();
  }, [isOpen, appointment, customer, stylist, isCorrectionMode, fetchAllActiveStaff, fetchBillingProcessors, fetchMembershipFee, tenantFetch, resetModalState, fetchBusinessDetails, fetchInventoryImpact]);
...
/** Handles adding a searched item to the bill. */
const handleAddItemToBill = useCallback((item: SearchableItem) => {
    ...
    if (item.type === 'product') {
        if (item.categoryName) displayName = `${item.categoryName} - ${displayName}`;
        if (item.unit) displayName = `${displayName} (${item.unit})`;
    }
    ...
    // Reset search and focus input for quick multi-item entry
    setSearchQuery('');
    setSearchResults([]);
    searchInputRef.current?.focus();
}, [billItems, customerIsMember, stylist._id, fetchInventoryImpact]);

/** Handles removing an item from the bill by its index. */
...
/** Handles changing the quantity of an item and recalculating its final price. */
    ...
    setBillItems(prev => prev.map((item, idx) => {
        if (idx === index) {
            ...
            return {
                ...
                finalPrice: unitPriceForCalc * newQuantity
            };
        }
        return item;
    }));
...
/** Handles changing the assigned staff member for a specific bill item. */
const handleItemStaffChange = useCallback((index: number, newStaffId: string) => {
    setBillItems(prev => prev.map((item, idx) => {
        if (idx === index) {
...
/** Handles updating payment details from the payment section. */
const handlePaymentChange = useCallback((method: keyof typeof newPaymentDetails, amount: string) => {
...
/** Handles the entire process of granting a new membership to the customer. */
const handleGrantMembership = useCallback(async () => {
    // 1. Validation
    if (isLoadingFee || membershipFee === null) { toast.error("Membership fee not configured."); return; }
    if (!membershipBarcode.trim() || !isBarcodeValid) { setError('Please enter a unique, valid barcode.'); return; }
    if (billItems.some(item => item.itemId === 'MEMBERSHIP_FEE_PRODUCT_ID')) { toast.info("Membership fee is already in the bill."); return; }
    
    // 2. API Call and State Update
    try {
        const response = await tenantFetch(`/api/customer/${customer._id}/toggle-membership`, {
            method: 'POST',
        ...
        if (result.success) {
            ...
            setIsGrantingMembership(false);
            toast.success(`Membership granted! Fee added to bill.`);

            const membershipFeeItem: BillLineItem = {
                itemType: 'fee',
                itemId: 'MEMBERSHIP_FEE_PRODUCT_ID',
...
}, [isLoadingFee, membershipFee, membershipBarcode, isBarcodeValid, billItems, tenantFetch, customer._id, stylist._id]);

/** Triggers the browser's print dialog for the receipt. */
const handlePrintReceipt = useCallback(() => window.print(), []);

/** The final action: validates the bill and submits it for payment and invoice creation. */
const handleFinalizeClick = useCallback(async () => {
    // 1. Validation Checks
    if (billItems.length === 0 || totals.trueGrandTotal < 0) {
    ...
    if (billItems.some(item => !item.staffId)) {
        setError('Please assign a staff member to every item in the bill.');
        toast.error('Please assign a staff member to every item.');
        return;
    }
    if (totals.balance > 0.01) {
        setError(`Payment amount (₹${totals.totalNewPaid.toFixed(2)}) is less than the amount due (₹${totals.displayTotal.toFixed(2)}).`);
        return;
    }
    ...
    setError(null);
    try {
        // Combine original payment details (for corrections) with new payments
        const finalPaymentDetails = {
            cash: (originalPaymentDetails.cash || 0) + newPaymentDetails.cash,
            card: (originalPaymentDetails.card || 0) + newPaymentDetails.card,
            upi: (originalPaymentDetails.upi || 0) + newPaymentDetails.upi,
            other: (originalPaymentDetails.other || 0) + newPaymentDetails.other,
        };
        // Adjust for cash change given back to the customer
        if (totals.changeDue > 0 && newPaymentDetails.cash > 0) {
            finalPaymentDetails.cash -= Math.min(newPaymentDetails.cash, totals.changeDue);
        }
  ...
  return {
    ...
    modalView,
    finalizedInvoiceData, // <<< THIS LINE WAS MISSING
    billItems,
    notes,
    newPaymentDetails,
    discount,
