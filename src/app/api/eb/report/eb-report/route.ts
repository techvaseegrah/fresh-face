...
// You correctly imported mongoose to access its types
...
export async function POST(request: NextRequest) {
  try {
    ...
    // This part is likely okay because .find() is more forgiving
    ...
    // 1. Explicitly convert the tenantId string into a valid Mongoose ObjectId.
    const tenantObjectId = new mongoose.Types.ObjectId(tenantId);

    // 2. Use this new ObjectId in the aggregation pipeline for a perfect type match.
    const appointments = await Appointment.aggregate([
        { 
          $match: { 
            tenantId: tenantObjectId, // Using the correct ObjectId type here
