...
interface StylistFromAPI { _id: string; staffInfo: {
    _id: string; // The linked Staff ID
    name: string;
  }; }
          ...
          <form onSubmit={handleSubmit} className="space-y-6 md:col-span-2 flex flex-col">
            <div className="space-y-6 flex-grow">
              ...
              <fieldset className={fieldsetClasses}>
                  ...
                  <div className="relative mt-5"><label htmlFor="stylist" className="block text-sm font-medium mb-1.5">Stylist <span className="text-red-500">*</span></label><select id="stylist" name="stylistId" value={formData.stylistId} onChange={handleChange} required disabled={formData.serviceIds.length===0||!formData.date||!formData.time||isLoadingStylists} className={`${inputBaseClasses} pr-8 disabled:bg-gray-100`}><option value="" disabled>{isLoadingStylists?'Checking...':'Select a stylist'}</option>{availableStylists.length>0?availableStylists.map(s=>(<option key={s._id} value={s._id}>{s.staffInfo.name}</option>)):<option value="" disabled>{!isLoadingStylists&&'No stylists available'}</option>}</select><div className="pointer-events-none absolute inset-y-0 right-0 top-6 flex items-center px-2"><ChevronDownIcon className="h-5 w-5 text-gray-400"/></div></div>
