...
export interface SalaryRecordType {
  id: string;
  staffId: string | { id: string; name: string; image?: string | null; position?: string; };
  month: string;
  year: number;
  baseSalary: number;
  otHours: number;
...
export interface StaffContextType {
  ...
  // FIX #1: Update the function's type definition here to accept the second argument
...
export const StaffProvider: React.FC<StaffProviderProps> = ({ children }) => {
  ...
  // --- Attendance (CheckInStaff function is now corrected) ---
  ...
  // FIX #2: Update the function implementation to accept requiredHours and send it in the body
  const checkInStaff = useCallback(async (staffId: string, requiredHours: number): Promise<AttendanceRecordTypeFE> => {
    ...
    try {
      const response = await fetch('/api/attendance?action=checkIn', {
        ...
        // Send both staffId and requiredHours in the request body
