// components/EditAppointmentForm.tsx - CORRECTED
'use client';
...
export default function EditAppointmentForm({
  ...
  // ... (all other state and the calculateTotals function remain the same) ...
  ...
  useEffect(() => {
    ...
    // 2. UPDATE HOW THE FORM'S INITIAL STATE IS SET
    const appointmentDate = new Date(appointment.appointmentDateTime);
    const datePart = appointmentDate.toISOString().split('T')[0]; // YYYY-MM-DD
    const timePart = appointmentDate.toTimeString().slice(0, 5); // HH:mm

    setFormData({
      serviceIds: appointment.serviceIds?.map(s => s._id) || [],
      stylistId: appointment.stylistId._id,
      date: datePart, // <-- Use the extracted date part
      time: timePart, // <-- Use the extracted time part
      notes: appointment.notes || '',
      status: appointment.status,
      // We don't need to set appointmentType here as the form field is disabled
    });
  ...
  // ... (The rest of the component, including findAvailableStylists, handlers, and JSX, remains the same) ...
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // IMPORTANT: We send the separate date and time fields. The backend API knows how to handle these.
    const updatePayload = {
      ...formData,
      appointmentType: appointment.appointmentType // ensure this is included
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                value={appointment.appointmentType} // Bind directly to appointment data
