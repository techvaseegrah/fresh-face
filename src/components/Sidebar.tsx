...
const Sidebar = ({ sidebarOpen, setSidebarOpen, isExpanded, setIsExpanded }) => {
  ...
  const navItems = useMemo((): NavItemConfig[] => {
    ...
    return [
      ...
      { href: '/reports', label: 'Reports', icon: <ChartBarIcon className="h-5 w-5" />, show: canSeeReports, subItems: reportSubItems.filter(item => item.show) },
      ...
      { href: '/back-office/reconciliation', label: 'Back Office', icon: <BriefcaseIcon className="h-5 w-5" />, show: canSeeReconciliation, subItems: reconciliationSubItems.filter(item => item.show) },
