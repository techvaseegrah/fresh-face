// src/components/Sidebar.tsx (Updated and Simplified)
...
import {
  ...
  LightBulbIcon, DocumentTextIcon, ShoppingCartIcon, BuildingStorefrontIcon, BanknotesIcon, BellAlertIcon,
} from '@heroicons/react/24/outline';
import { BeakerIcon } from 'lucide-react';

const Sidebar = () => {
  const pathname = usePathname();
  // We can still use useSession to get user data, but we don't need to check `status` anymore.
  const { data: session } = useSession();

  // If for some reason the session is missing (it shouldn't be), we can return null as a safeguard.
  if (!session) {
    return null;
  }
  ...
  // ... (All your permission and nav item logic is perfectly fine and does not need to change) ...
  const canAccessDashboard = hasAnyPermission(userPermissions, [PERMISSIONS.DASHBOARD_READ, PERMISSIONS.DASHBOARD_MANAGE]);
  const canAccessAppointments = hasAnyPermission(userPermissions, [PERMISSIONS.APPOINTMENTS_READ, PERMISSIONS.APPOINTMENTS_CREATE,PERMISSIONS.APPOINTMENTS_MANAGE]);
  const canAccessCustomers = hasAnyPermission(userPermissions, [PERMISSIONS.CUSTOMERS_READ, PERMISSIONS.CUSTOMERS_CREATE, PERMISSIONS.CUSTOMERS_MANAGE]);
  const canAccessShop = hasAnyPermission(userPermissions, [PERMISSIONS.PRODUCTS_READ, PERMISSIONS.SERVICES_READ]);
  ...
  const canAccessProcurement = hasAnyPermission(userPermissions, [PERMISSIONS.PROCUREMENT_READ, PERMISSIONS.PROCUREMENT_CREATE]);
  ...
  const canAccessUsers = hasAnyPermission(userPermissions, [PERMISSIONS.USERS_READ,PERMISSIONS.USERS_MANAGE]);
  const canAccessRoles = hasAnyPermission(userPermissions, [PERMISSIONS.ROLES_READ, PERMISSIONS.ROLES_MANAGE]);
  ...
  const navItems = [
    ...
    { href: '/alerts', label: 'Alerts', icon: BellAlertIcon, show: canAccessAlerts },
  ...
  // **THE LAYOUT FIX**:
  // No `fixed` positioning. `flex-shrink-0` ensures it doesn't get squished.
  return (
    <aside className="w-64 h-screen bg-white text-black shadow-lg flex flex-col flex-shrink-0">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
        <nav className="p-4 space-y-1">
          {visibleNavItems.map((item) => {
            ...
            return (
              <Link key={item.label} href={item.href} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-gray-700 ${isActive ? 'bg-gray-100 text-black font-medium' : 'hover:bg-gray-50 hover:text-black'}`}>
